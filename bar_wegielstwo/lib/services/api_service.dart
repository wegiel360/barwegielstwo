...
class ApiService {
  ...
  Future<void> updateOrderStatus(String orderId, String status, {String? reason}) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd aktualizacji zamówienia');
    }
  }
  ...
  Future<void> deleteOrder(String orderId) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd usuwania zamówienia');
    }
  }

  Future<void> clearAllOrders() async {
    final resp = await _client.delete(
      Uri.parse(_url('${ApiConfig.ordersEndpoint}/clear')),
    );
    if (resp.statusCode != 200) {
      throw Exception('Błąd czyszczenia zamówień');
    }
  }
  ...
  Future<MenuItemModel> addMenuItem(String name, String emoji, String category) async {
    final body = jsonEncode({
      'name': name,
      'emoji': emoji,
      'category': category,
  ...
  Future<void> updateMenuItem(String name, Map<String, dynamic> updates) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd aktualizacji menu');
    }
  }
  ...
  Future<void> deleteMenuItem(String name) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd usuwania z menu');
    }
  }

  Future<List<String>> getPortions() async {
    final resp = await _client.get(
      Uri.parse(_url(ApiConfig.portionsEndpoint)),
    );
    if (resp.statusCode == 200) {
      final data = jsonDecode(resp.body) as Map<String, dynamic>;
      return (data['portions'] as List<dynamic>).map((e) => e.toString()).toList();
    }
    throw Exception('Błąd pobierania porcji');
  }
  ...
  Future<List<ExtraModel>> addExtra(String name, {String emoji = ''}) async {
  ...
  Future<void> deleteExtra(String name) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd usuwania dodatku');
    }
  }
  ...
  Future<void> updateExtra(String name, {bool? available}) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd aktualizacji dodatku');
    }
  }
  ...
  Future<void> triggerDzwonek() async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd wywołania dzwonka');
    }
  }
  ...
  Future<void> setCustomMessage(String message) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd zapisu wiadomości');
    }
  }
  ...
  Future<void> setDanieDnia(String danie) async {
    ...
    if (resp.statusCode != 200) {
      throw Exception('Błąd zapisu dania dnia');
    }
  }

  Future<void> requestCleanup() async {
    final resp = await _client.post(
      Uri.parse(_url(ApiConfig.cleanupEndpoint)),
    );
    if (resp.statusCode != 200) {
      throw Exception('Błąd czyszczenia');
    }
  }

  Future<Map<String, dynamic>> getDatabaseRaw() async {
    final resp = await _client.get(
      Uri.parse(_url(ApiConfig.databaseEndpoint)),
    );
    if (resp.statusCode == 200) {
      return jsonDecode(resp.body) as Map<String, dynamic>;
    }
    throw Exception('Błąd pobierania bazy danych');
  }
}