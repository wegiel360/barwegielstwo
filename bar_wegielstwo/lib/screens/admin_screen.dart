...
class _AdminScreenState extends State<AdminScreen> with TickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: _currentTab == 0 ? _buildOrdersTab(ordersProvider) : _buildMenuTab(menuProvider),
          ),
        ],
      ),
    );
      ...
      children: [
            ...
            children: [
              ...
              Expanded(
                child: OutlinedButton.icon(
                  ...
                  label: const Text('Wyczyść', style: TextStyle(fontSize: 12)),
                  style: OutlinedButton.styleFrom(
        ...
        Expanded(
          child: orders.isEmpty
              ? const Center(
                  child: Text('Brak zamówień', style: TextStyle(color: Colors.white54)),
                )
              : RefreshIndicator(
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  filled: true,
          fillColor: const Color(0xFF2A1A16).withAlpha(128),
      ...
      children: [
        ...
        Expanded(
          child: menuProvider.extras.isEmpty
              ? const Center(child: Text('Brak dodatków', style: TextStyle(color: Colors.white54)))
              : ListView.builder(
                  padding: const EdgeInsets.symmetric(horizontal: 8),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: Text(extra.emoji.isEmpty ? '➕' : extra.emoji, style: const TextStyle(fontSize: 20)),
  ...
  void _addMenuItem() {
    ...
    if (name.isEmpty || emoji.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Podaj nazwę i emoji'), backgroundColor: Colors.red),
      );
      return;
    }
  ...
  void _deleteMenuItem(MenuProvider menuProvider, String name) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF2A1A16),
        title: Text('Usunąć "$name"?', style: const TextStyle(color: Colors.white)),
        actions: [
          TextButton(
  ...
  void _deleteExtra(MenuProvider menuProvider, String name) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF2A1A16),
        title: Text('Usunąć dodatek "$name"?', style: const TextStyle(color: Colors.white)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('Anuluj', style: TextStyle(color: Colors.white)),
  ...
  void _clearAllOrders() {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF2A1A16),
        title: const Text('Wyczyść wszystkie?', style: TextStyle(color: Color(0xFFF87171))),
        content: const Text('Czy na pewno chcesz usunąć wszystkie zamówienia?', style: TextStyle(color: Colors.white)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('Anuluj', style: TextStyle(color: Colors.white)),
