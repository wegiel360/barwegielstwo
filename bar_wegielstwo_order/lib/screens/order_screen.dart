...
class _OrderScreenState extends State<OrderScreen> {
                ...
                children: [
                  ...
                  if (_showPortions && _selectedItem != null) ...[
                    ...
                    Wrap(
                      spacing: 6,
                      runSpacing: 6,
                      children: ['1 porcja', '2 porcje', 'Pół porcji']
                          .map((p) => ChoiceChip(
                        ...
                        Expanded(
                          child: FilledButton(
                            onPressed: () {
                              ...
                              if (_selectedItem!.category == 'Napoje') {
                                _addToCartNapoje();
                              } else {
                                _addToCart();
                              }
  ...
  void _addToCart() {
    ...
    context.read<CartProvider>().addItem(OrderItem(
      name: item.name,
      emoji: item.emoji,
  ...
  String _generateReceipt(OrderModel order) {
    final buf = StringBuffer();
    buf.writeln('=== Bar Węgielstwo ===');
    buf.writeln('Zamówienie #${order.orderNumber}');
    buf.writeln('Status: ${order.status}');
    buf.writeln('Klient: ${order.customerName}');
    buf.writeln('---');
    for (final item in order.items) {
      buf.writeln('${item.emoji} ${item.name} (${item.portion})');
      if (item.extras.isNotEmpty) {
        buf.writeln('  + ${item.extras.join(', ')}');
      }
    }
    buf.writeln('---');
    if (order.notes.isNotEmpty) {
      buf.writeln('Uwagi: ${order.notes}');
    }
    buf.writeln('Czas: ${order.timestamp.isNotEmpty ? order.timestamp : 'teraz'}');
    return buf.toString();
  }
}